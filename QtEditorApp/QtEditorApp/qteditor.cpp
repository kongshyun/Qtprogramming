...
void QtEditor::openFile( )
{
    ...
    if(fileInfo.isReadable( )) {
        ...
        textedit->setWindowTitle(filename);		           // Save 기능을 위해 경로 저장
        windowHash.key(textedit)->setText(filename); 		// QAction의 text 변경

        if(fileInfo.suffix( ) == "htm" || fileInfo.suffix( ) == "html")
            textedit->setHtml(msg);				// 서식 있는 문자열일 때
        else
...
void QtEditor::saveFile( ) {
    ...
    if(window != nullptr) {
        ...
        QString filename = textedit->windowTitle( );	// 기존의 파일명은 타이틀 바에 있음
        if(!filename.length( )) {		// 파일명이 없을 때 사용자로부터 받아온다.
            filename = QFileDialog::getSaveFileName(this, "Select a file to save", ".",
                                                               "Text File(*.text *.txt *.html *.htm *.c *.cpp *.h)");
            if(!filename.length( )) return;
            textedit->setWindowTitle(filename);	 // 다음 save를 위해 title바에 설정
            windowHash.key(textedit)->setText(filename); 	// QAction의 text 변경
        }
...
void QtEditor::saveasFile( )
{
    ...
    if(window != nullptr) {
        QTextEdit *textedit = qobject_cast<QTextEdit*>(window->widget( ));
        QString filename = QFileDialog::getSaveFileName(this, "Select a file to save", ".", "Text File(*.text *.txt *.html *.htm *.c *.cpp *.h)");
            if(!filename.length( )) return;
            textedit->setWindowTitle(filename);	 // 다음 save를 위해 title바에 설정
            windowHash.key(textedit)->setText(filename); 	// QAction의 text 변경
        QFile file(filename);		// 파일 객체 생성
